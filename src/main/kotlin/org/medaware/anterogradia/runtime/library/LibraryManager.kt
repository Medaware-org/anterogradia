...
class LibraryManager {
    ...
    private fun libSanityCheck(lib: Class<*>) {
        ...
        try {
            ...
            throw SanityException("Sanity check failed for '${lib.canonicalName}': The class is expected to have a nullary constructor.")
        }
        ...
        lib.declaredMethods.forEach {
            ...
            if (it.returnType != String::class.java)
                throw SanityException("All Anterogradia @Function-s must return a String value. Function '${function.identifier}' (${it.name}) violates this rule by returning '${it.returnType.canonicalName}'.")
            ...
            function.params.forEach { paramId ->
                if (params.contains(paramId))
                    throw SanityException("Sanity check failed for '${lib.canonicalName}': Parameter names of function '${function.identifier}' (${it.name}) overlap on parameter '${it}'.")
                params.add(paramId)
            }

            if (!registeredFunctions.add(function.identifier to params.size))
                throw SanityException("Sanity check failed for '${lib.canonicalName}': Function name and parameter count overlaps for function '${function.identifier}' (${it.name}).")
            ...
            if (it.parameters.any { it.type != String::class.java })
                throw SanityException("Sanity check failed for '${lib.canonicalName}': All function parameters must be of type String. This rule is violated in function '${function.identifier}' (${it.name}).")

            if (it.parameters.size != params.size)
                throw SanityException("Sanity check failed for '${lib.canonicalName}': The length of the parameter list does not match the method's parameters of function '${function.identifier}' (${it.name}): ${it.parameters.size} parameters in the definition, ${function.params.size} parameters declared in the annotation.")
        }
    }
    ...
    fun register(libClass: Class<*>) {
        ...
        if (conflict != null)
            throw LibraryException("Failed to register library class '${libClass.canonicalName}' due to a namespace conflict with '${conflict.canonicalName}'.")

        libSanityCheck(libClass)

        if (!libraries.add(libClass))
            throw LibraryException("Attempted to register the same library class twice: '${libClass.canonicalName}'.")

        logger.info("Successfully registered library '${libClass.canonicalName}' with prefix '${lib.prefix}'.")
    }

    private fun libMethod(prefix: String, name: String, args: HashMap<String, Node>, runtime: Runtime): String {
        val lib =
        ...
        if (methods.isEmpty())
            throw FunctionCallException("Could not find function $prefix:'$name' in library '${lib.canonicalName}'.")
        ...
        if (method.size == 1) {
            ...
            return method.first().first.invoke(instance, groupedParams.toTypedArray()) as String
        }

        if (method.size > 1)
            throw FunctionCallException("There is more than one potential match for function $prefix:'$name'.")

        throw FunctionCallException("Could not find function $prefix:'$name'.")
    }
